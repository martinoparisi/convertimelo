  ...
  template: `
    <div class="space-y-6">
      <div class="bg-white/80 dark:bg-slate-800/50 backdrop-blur-sm shadow-lg border border-gray-200 dark:border-indigo-500/20 rounded-lg p-6 transition-all duration-300 hover:shadow-[0_0_20px_rgba(79,70,229,0.15)]">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white drop-shadow-[0_0_5px_rgba(139,92,246,0.5)]">Bentornato!</h2>
        <p class="mt-2 text-gray-600 dark:text-gray-300">Seleziona uno strumento dalla barra di navigazione per iniziare.</p>
      </div>

      <div class="bg-white/80 dark:bg-slate-800/50 backdrop-blur-sm shadow-lg border border-gray-200 dark:border-indigo-500/20 rounded-lg p-6 transition-all duration-300 hover:shadow-[0_0_20px_rgba(79,70,229,0.15)]">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg font-medium text-gray-900 dark:text-white drop-shadow-[0_0_5px_rgba(139,92,246,0.5)]">Attività Recente</h3>
          <button
            (click)="clearHistory()"
            class="text-red-600 dark:text-red-400 hover:text-red-500 dark:hover:text-red-300 text-sm flex items-center gap-1 transition-colors px-3 py-1 rounded-md hover:bg-red-50 dark:hover:bg-red-500/10"
            *ngIf="(history$ | async)?.length"
          >
            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6M9 7V5a2 2 0 012-2h2a2 2 0 012 2v2" />
            </svg>
            Svuota
          </button>
        </div>
          ...
          <ul class="-mb-8">
            <li *ngFor="let item of history$ | async">
              <div class="relative pb-8">
                <span class="absolute top-4 left-4 -ml-px h-full w-0.5 bg-gray-200 dark:bg-indigo-500/20" aria-hidden="true"></span>
                <div class="relative flex space-x-3">
                  <div>
                    <span class="h-8 w-8 rounded-full bg-indigo-600 flex items-center justify-center ring-8 ring-white dark:ring-slate-900 shadow-[0_0_10px_rgba(79,70,229,0.5)]">
                      <!-- Icon based on type -->
                  ...
                  <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                    <div>
                      <p class="text-sm text-gray-600 dark:text-gray-300">{{ item.details }}</p>
                    </div>
