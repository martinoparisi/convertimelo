    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 transition-colors duration-200">
      ...
      <div class="space-y-6">
        <div>
          ...
          <textarea [(ngModel)]="prompt" rows="4" class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm p-2 border" placeholder="Descrivi cosa vuoi generare..."></textarea>
        </div>

        <button (click)="generate()" [disabled]="loading || !prompt" class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50">
          <span *ngIf="loading">Generazione in corso...</span>
          <span *ngIf="!loading">Genera Contenuto</span>
        </button>

        <div *ngIf="result" class="mt-6">
          <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Output Generato</label>
...
export class GenkitComponent {
  ...
  async generate() {
    ...
    try {
      ...
      this.historyService.addEntry('genkit', `Generated content for prompt: ${this.prompt.substring(0, 50)}...`);
    } catch (err: any) {
